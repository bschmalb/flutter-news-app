...
class ArticlePreviewStore {
  ...
  Map<int, ArticlePreviewModel?> peekMany(Iterable<int> articleIds) {
    return {
      for (final articleId in articleIds) articleId: _cache[articleId],
    };
  }
  ...
  Future<ArticlePreviewModel?> fetch(int articleId, {bool refresh = false}) {
    ...
    final future = () async {
      try {
        ...
        final hydratedArticle = await _hydrateAuthors(
          article,
          refresh: refresh,
        );
        ...
        log(
          'Failed to fetch article preview for $articleId: $error',
  ...
  Future<Map<int, ArticlePreviewModel?>> fetchMany(
    Iterable<int> articleIds, {
    bool refresh = false,
  }) async {
    final uniqueArticleIds = articleIds.toSet().toList(growable: false);
    final results = await Future.wait(
      uniqueArticleIds.map(
        (articleId) async => MapEntry(
          articleId,
          await fetch(articleId, refresh: refresh),
        ),
      ),
    );
  ...
  Future<ArticlePreviewModel> _hydrateAuthors(
    ...
    final authorsById = await authorStore.fetchMany(
      article.authorIds,
      refresh: refresh,
    );
